...
def main() -> None:
    with HuBMAPClient(timeout=45.0) as client:
        records = client.search_datasets(status="Published", limit=3)

    if not records:
        raise SystemExit("HuBMAP returned no published datasets")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        write_html_report(
            records,
            destination,
            query={"source": "HuBMAP", "status": "Published", "limit": 3},
        )
